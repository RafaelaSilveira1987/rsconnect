                    ...
                    <?php if ($isPreSchedule): ?>
                        ...
                        <?php if (!empty($appointment['availability_status'])): ?>
                            <div class="pre-schedule-note ready">
                                ...
                                <span><?= View::e(match ($appointment['availability_status']) { 'requested' => 'consulta solicitada', 'sent' => 'enviada ao n8n', 'received' => 'horários recebidos', 'empty' => 'sem horários livres', 'failed' => 'falha ao buscar', 'hold_requested' => 'pré-reserva solicitada ao Google', 'slot_selected' => 'horário validado/escolhido', default => $appointment['availability_status'] }) ?><?= !empty($appointment['availability_slot_count']) ? ' · ' . (int) $appointment['availability_slot_count'] . ' opção(ões)' : '' ?></span>
