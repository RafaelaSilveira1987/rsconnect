...
<section class="page-header">
    <div><span class="eyebrow">Financeiro SaaS</span><h1>Inscrição pública e trial Asaas</h1><p>Disponibilize somente o Plano Inicial no login, com cartão obrigatório e primeira cobrança após o período gratuito.</p></div>
...
<div class="settings-grid public-signup-admin-grid">
    ...
    <aside class="card public-signup-checklist">
        ...
        <?php $selected = null; foreach ($gateways as $gateway) { if ((int) $gateway['id'] === (int) ($settings['gateway_id'] ?? 0)) $selected = $gateway; } ?>
        <ul>
...
<section class="card mt-24">
    ...
    <div class="table-responsive"><table><thead><tr><th>Empresa</th><th>Responsável</th><th>Status</th><th>Trial</th><th>Conta</th><th>Criado em</th></tr></thead><tbody>
    <?php if ($recent === []): ?><tr><td colspan="6" class="empty-state">Nenhuma inscrição iniciada.</td></tr><?php else: foreach ($recent as $row): ?>
        <tr><td><strong><?= View::e((string) $row['company_name']) ?></strong><small><?= View::e((string) $row['email']) ?></small></td><td><?= View::e((string) $row['responsible_name']) ?></td><td><span class="status-pill status-<?= View::e((string) $row['status']) ?>"><?= View::e((string) $row['status']) ?></span><?php if (!empty($row['last_error'])): ?><small title="<?= View::e((string) $row['last_error']) ?>">Ver erro no título</small><?php endif; ?></td><td><?= View::e(date('d/m/Y', strtotime((string) $row['trial_ends_at']))) ?></td><td><?= !empty($row['tenant_name']) ? View::e((string) $row['tenant_name']) : '—' ?></td><td><?= View::e(date('d/m/Y H:i', strtotime((string) $row['created_at']))) ?></td></tr>
