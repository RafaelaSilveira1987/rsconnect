...
<div class="agent-lab-page" data-agent-lab data-simulate-url="<?= View::e(Router::url('/agent-tests/simulate')) ?>" data-csrf="<?= View::e($csrfToken) ?>">
    <section class="card">
        <div class="agent-lab-hero">
            <div>
                <span class="eyebrow">Homologação dos assistentes</span>
            ...
            <form class="agent-lab-selector" method="get" action="<?= View::e(Router::url('/agent-tests')) ?>">
                ...
                <label class="field"><span>Assistente</span><select name="agent_id" required onchange="this.form.submit()">
                    <?php foreach ($agents as $agent): ?><option value="<?= (int) $agent['id'] ?>" <?= $selectedAgentId === (int) $agent['id'] ? 'selected' : '' ?>><?= View::e((string) $agent['name']) ?> · <?= View::e((string) $agent['model_name']) ?></option><?php endforeach; ?>
                </select></label>
                <a class="btn btn-quiet" href="<?= View::e(Router::url('/agents?tenant_id=' . $selectedTenantId)) ?>">Abrir assistente</a>
            </form>
        </div>
    </section>
    ...
    <section class="agent-lab-grid">
        <article class="card agent-lab-chat">
            <div class="section-heading"><div><span class="eyebrow">Simulador</span><h3>Conversa de teste</h3><p>O WhatsApp não é acionado. No modo IA real, a resposta usa a mesma chave e o mesmo prompt configurados no assistente.</p></div><button class="btn btn-quiet" type="button" data-lab-reset>Limpar conversa</button></div>
