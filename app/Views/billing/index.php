...
<section class="admin-module-hero billing-hero-v2">
    ...
    <div class="admin-module-hero-actions">
        <button class="btn btn-primary" type="button" data-payment-link-open data-toggle-panel="payment-link-drawer">Gerar link de pagamento</button>
            ...
            <?php foreach ($tenants as $tenant): ?>
                ...
                <article class="billing-subscription-card <?= $isBlocked ? 'is-access-blocked' : '' ?>">
                    ...
                    <footer>
                        ...
                        <button class="btn btn-small <?= $isBlocked ? 'btn-primary' : 'btn-primary-soft' ?>" type="button" data-toggle-panel="subscription-drawer" data-subscription-open="edit" data-subscription-auto-open="<?= $autoEditSubscription && $selectedTenantId === (int) $tenant['id'] ? '1' : '0' ?>" data-subscription-id="<?= (int) ($tenant['subscription_id'] ?? 0) ?>" data-tenant-id="<?= (int) $tenant['id'] ?>" data-plan-id="<?= (int) ($tenant['plan_id'] ?? 0) ?>" data-billing-status="<?= View::e((string) ($tenant['billing_status'] ?? 'active')) ?>" data-billing-cycle="<?= View::e((string) ($tenant['billing_cycle'] ?? 'monthly')) ?>" data-amount="<?= View::e(number_format((float) ($tenant['amount'] ?? 0), 2, ',', '.')) ?>" data-period-start="<?= View::e((string) ($tenant['current_period_starts_at'] ?? '')) ?>" data-period-end="<?= View::e((string) ($tenant['current_period_ends_at'] ?? '')) ?>" data-next-billing="<?= View::e((string) ($tenant['next_billing_at'] ?? '')) ?>" data-trial-end="<?= View::e((string) ($tenant['trial_ends_at'] ?? '')) ?>" data-notes="<?= View::e(rawurlencode((string) ($tenant['notes'] ?? ''))) ?>" data-tenant-name="<?= View::e((string) $tenant['name']) ?>" data-access-message="<?= View::e(rawurlencode((string) ($access['message'] ?? ''))) ?>"><?= $isBlocked ? 'Editar vigência e liberar' : 'Editar vigência' ?></button>
        ...
        <div class="billing-invoice-list">
            <?php foreach ($invoices as $invoice): ?>
                ...
                <article class="billing-invoice-card">
                    ...
                    <div class="billing-invoice-status"><span class="badge badge-<?= View::e($invoice['status']) ?>"><?= View::e($statusLabel[$invoice['status']] ?? $invoice['status']) ?></span></div>
        ...
        <div class="admin-module-card-list compact-cards">
            <?php foreach ($plans as $plan): ?>
                <article class="admin-record-card">
                    ...
                    <dl class="admin-record-metrics"><div><dt>Usuários</dt><dd><?= View::e((string) ($plan['limits']['users'] ?? '∞')) ?></dd></div><div><dt>WhatsApps</dt><dd><?= View::e((string) ($plan['limits']['instances'] ?? '∞')) ?></dd></div><div><dt>Assistentes</dt><dd><?= View::e((string) ($plan['limits']['agents'] ?? '∞')) ?></dd></div><div><dt>Mensagens</dt><dd><?= View::e((string) ($plan['limits']['messages_month'] ?? '∞')) ?></dd></div></dl>
