...
<section class="admin-module-hero billing-hero-v2">
    ...
    <div class="admin-module-hero-actions">
        ...
        <a class="btn btn-quiet" href="<?= View::e(Router::url('/payment-gateways')) ?>">Configurar meios de pagamento</a>
    </div>
</section>
            ...
            <?php foreach ($tenants as $tenant): ?>
                ...
                <article class="billing-subscription-card <?= $isBlocked ? 'is-access-blocked' : '' ?>">
                    ...
                    <div class="billing-subscription-facts">
                        ...
                        ?>
                        <div><span>IA no mês</span><strong><?= (int) ($tenantAiUsage['total'] ?? 0) ?> interação(ões) entregues</strong><small>Limite de IA pago pela RS: <?= $tenantAiUsed ?> / <?= $tenantAiLimit === null ? '∞' : (int) $tenantAiLimit ?><?= $tenantAiLimit === null ? '' : ' (' . $tenantAiPercent . '%)' ?> · chave própria do cliente: <?= (int) ($tenantAiUsage['tenant'] ?? 0) ?> · mensagens: <?= (int) ($tenantAiUsage['messages']['total'] ?? 0) ?></small></div>
                    </div>
            ...
            <?php foreach ($invoices as $invoice): ?>
                ...
                <article class="billing-invoice-card">
                    ...
                    <div class="billing-invoice-actions">
                        ...
                        <form class="admin-inline-status-form billing-status-form" method="post" action="<?= View::e(Router::url('/billing/invoices/status')) ?>"><?= Csrf::input() ?><input type="hidden" name="invoice_id" value="<?= (int) $invoice['id'] ?>"><label class="billing-status-field"><span>Situação da cobrança</span><select name="status"><option value="open" <?= $invoice['status'] === 'open' ? 'selected' : '' ?>>Aberta</option><option value="paid" <?= $invoice['status'] === 'paid' ? 'selected' : '' ?>>Paga</option><option value="overdue" <?= $invoice['status'] === 'overdue' ? 'selected' : '' ?>>Atrasada</option><option value="cancelled" <?= $invoice['status'] === 'cancelled' ? 'selected' : '' ?>>Cancelada</option></select></label><button class="btn btn-small btn-outline" type="submit">Salvar situação</button></form>
                    </div>
                </article>
            <?php endforeach; ?>
...
<aside class="conversation-details conversation-drawer admin-form-drawer" id="external-charge-drawer" aria-label="Importar cobrança externa" aria-modal="true" role="dialog"><div class="conversation-drawer-header"><div><span class="eyebrow">Cobrança existente</span><h2>Usar cobrança já criada</h2><p>Vincule ao RS Connect uma cobrança já criada na InfinitePay ou em outro serviço.</p></div><button class="icon-button drawer-close" type="button" data-close-panel="external-charge-drawer">×</button></div><div class="conversation-drawer-body"><form class="drawer-form" method="post" action="<?= View::e(Router::url('/payment-gateways/invoices/import-external')) ?>"><?= Csrf::input() ?><input type="hidden" name="return_to" value="/billing"><section class="drawer-section"><div class="drawer-form-grid"><label class="field drawer-span"><span>Cobrança do RS Connect</span><select name="invoice_id" required><option value="">Selecione</option><?php foreach ($payableInvoices as $invoice): ?><option value="<?= (int) $invoice['id'] ?>"><?= View::e($invoice['tenant_name']) ?> · <?= View::e($invoice['invoice_number']) ?> · <?= View::e($money($invoice['amount'])) ?></option><?php endforeach; ?></select></label><label class="field"><span>Serviço</span><select name="provider"><option value="infinitepay">InfinitePay</option><option value="external">Outro serviço externo</option></select></label><label class="field"><span>Situação atual</span><select name="status"><option value="open">Aguardando pagamento</option><option value="paid">Paga</option><option value="overdue">Vencida</option><option value="cancelled">Cancelada</option></select></label><label class="field drawer-span"><span>Identificador da cobrança externa</span><input name="external_id" placeholder="Código exibido no serviço" required></label><label class="field drawer-span"><span>Link de pagamento</span><input name="checkout_url" type="url" placeholder="https://..." required><small class="field-hint">O RS Connect armazenará e enviará esse link; não criará uma nova cobrança no serviço.</small></label></div></section><div class="operations-alert is-info"><strong>Atualização automática opcional</strong><p>Um fluxo n8n pode enviar o status normalizado para <code>/webhooks/payments/infinitepay</code> usando a chave de segurança configurada no meio de pagamento.</p></div><div class="drawer-savebar"><button class="btn btn-quiet" type="button" data-close-panel="external-charge-drawer">Cancelar</button><button class="btn btn-primary" type="submit" <?= !$payableInvoices ? 'disabled' : '' ?>>Vincular cobrança</button></div></form></div></aside>

<aside class="conversation-details conversation-drawer admin-form-drawer" id="payment-link-drawer" aria-label="Gerar link de pagamento" aria-modal="true" role="dialog"><div class="conversation-drawer-header"><div><span class="eyebrow">Pagamento</span><h2>Gerar link de pagamento</h2><p>Escolha uma cobrança pendente e o meio de pagamento que concluirá a cobrança.</p></div><button class="icon-button drawer-close" type="button" data-close-panel="payment-link-drawer">×</button></div><div class="conversation-drawer-body"><form class="drawer-form" method="post" action="<?= View::e(Router::url('/payment-gateways/invoices/create-link')) ?>" data-payment-link-form><?= Csrf::input() ?><input type="hidden" name="return_to" value="/billing"><section class="drawer-section"><div class="drawer-form-grid"><label class="field drawer-span"><span>Cobrança pendente</span><select name="invoice_id" required data-payment-link-invoice><option value="">Selecione</option><?php foreach ($payableInvoices as $invoice): ?><option value="<?= (int) $invoice['id'] ?>"><?= View::e($invoice['tenant_name']) ?> · <?= View::e($invoice['invoice_number']) ?> · <?= View::e($money($invoice['amount'])) ?> · vence <?= View::e($date($invoice['due_date'])) ?></option><?php endforeach; ?></select></label><label class="field drawer-span"><span>Meio de pagamento</span><select name="gateway_id"><option value="0">Usar meio de pagamento padrão</option><?php foreach ($paymentGateways as $gateway): ?><option value="<?= (int) $gateway['id'] ?>"><?= View::e($gateway['label']) ?><?= !empty($gateway['is_default']) ? ' · padrão' : '' ?></option><?php endforeach; ?></select></label><label class="field drawer-span"><span>Forma de pagamento</span><select name="payment_method"><option value="">Usar padrão do meio de pagamento</option><?php foreach ($paymentMethodLabels as $key => $label): ?><option value="<?= View::e($key) ?>"><?= View::e($label) ?></option><?php endforeach; ?></select></label></div></section><?php if (!$paymentGateways): ?><div class="operations-alert is-warning"><strong>Nenhum meio de pagamento ativo</strong><p>Cadastre ou ative um meio de pagamento antes de gerar o link.</p><a class="btn btn-small btn-outline" href="<?= View::e(Router::url('/payment-gateways')) ?>">Configurar meio de pagamento</a></div><?php endif; ?><div class="drawer-savebar"><button class="btn btn-quiet" type="button" data-close-panel="payment-link-drawer">Cancelar</button><button class="btn btn-primary" type="submit" <?= !$paymentGateways || !$payableInvoices ? 'disabled' : '' ?>>Gerar link</button></div></form></div></aside>
