    ...
    <?php if (Auth::can('agents.manage')): ?>
        <aside class="stack">
            <form class="card sticky-card" method="post" action="<?= View::e(Router::url('/agents')) ?>">
                ...
                <div class="form-grid two"><label class="field"><span>Provedor</span><select name="model_provider"><option value="google">Google Gemini</option><option value="openai">OpenAI em breve</option><option value="anthropic">Anthropic em breve</option><option value="custom">Custom em breve</option></select></label><label class="field"><span>Temperatura</span><input type="number" name="temperature" value="0.2" min="0" max="1" step="0.1"></label></div>
                <label class="field"><span>Modelo</span><input name="model_name" value="gemini-2.0-flash" required></label>
