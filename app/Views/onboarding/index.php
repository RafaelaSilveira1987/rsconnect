            ...
            <section class="card onboarding-step-panel <?= View::e($statusClass((string) $step['status'])) ?>" id="<?= View::e($step['key']) ?>">
                ...
                <?php elseif ($step['key'] === 'attendance_rules'): ?>
                    <form class="wizard-card onboarding-inline-form" method="post" action="<?= View::e(Router::url('/onboarding/attendance')) ?>">
                        ...
                        <label class="field"><span>Intervalo mínimo entre respostas (seg.)</span><input type="number" name="cooldown_seconds" min="0" max="3600" value="<?= (int) ($defaultAgent['cooldown_seconds'] ?? 10) ?>"><small class="field-hint">Evita respostas automáticas muito próximas. A última mensagem pendente pode ser reprocessada ao salvar o assistente.</small></label>
