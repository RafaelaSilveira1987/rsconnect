...
final class OperationalPlaybookService
{
    ...
    private function normalizeKey(string $key): string
    {
        ...
        if (str_starts_with($key, 'access.')) return 'access';
    ...
    private function playbooks(): array
    {
        return [
            ...
            'openai' => ['title'=>'O assistente de IA precisa de revisão','cause'=>'O serviço de IA não confirmou uma resposta válida.','impact'=>'Os assistentes podem parar de responder até a chave de acesso ou o serviço voltar ao normal.','steps'=>['Abra Assistentes e chaves de acesso.','Revise o último problema informado pelo serviço de IA.','Corrija a chave ou o limite de uso e tente novamente nas conversas pendentes.'],'actions'=>[['label'=>'Abrir chaves de acesso','url'=>'/ai-credentials'],['label'=>'Ver fila da IA','url'=>'/central-operacao?tab=ai_reprocess']]],
            'openai_401' => ['title'=>'OpenAI: chave inválida ou revogada','cause'=>'Retorno HTTP 401 do provedor.','impact'=>'As respostas de IA ficam indisponíveis para a credencial afetada.','steps'=>['Confirme a chave cadastrada.','Troque uma chave revogada ou incorreta.','Teste novamente e reprocese a fila.'],'actions'=>[['label'=>'Abrir chaves de acesso','url'=>'/ai-credentials']]],
            'openai_quota' => ['title'=>'OpenAI: crédito ou quota indisponível','cause'=>'O retorno indica limite financeiro/quota da conta.','impact'=>'A IA não consegue gerar novas respostas até a conta voltar a ter capacidade.','steps'=>['Confira faturamento/limites no provedor.','Regularize crédito ou limite da conta.','Execute nova verificação antes de reprocessar a fila.'],'actions'=>[['label'=>'Abrir chaves de acesso','url'=>'/ai-credentials']]],
            'openai_429' => ['title'=>'OpenAI: limite de requisições atingido','cause'=>'Retorno HTTP 429.','impact'=>'Respostas podem atrasar ou falhar temporariamente.','steps'=>['Confirme o limite da conta/modelo.','Reduza picos de requisições ou aguarde a janela do provedor.','Reprocese apenas as mensagens pendentes após normalização.'],'actions'=>[['label'=>'Abrir chaves de acesso','url'=>'/ai-credentials']]],
            'openai_403' => ['title'=>'OpenAI: acesso negado','cause'=>'Retorno HTTP 403 do provedor ou intermediário.','impact'=>'A credencial/rede atual não consegue acessar o recurso solicitado.','steps'=>['Revise organização, projeto, endpoint e permissões da chave.','Confirme se proxy/intermediário não está bloqueando a chamada.','Teste novamente pela tela de credenciais.'],'actions'=>[['label'=>'Abrir chaves de acesso','url'=>'/ai-credentials']]],
            ...
            'access' => ['title'=>'Acesso da empresa requer revisão','cause'=>'O controle de acesso confirmou bloqueio por assinatura, vigência, teste ou cobrança vencida além da tolerância.','impact'=>'Usuários e automações da empresa podem ficar impedidos de usar o RS Connect.','steps'=>['Abra Assinaturas e localize a empresa indicada.','Confira situação da assinatura, fim da vigência e cobranças vencidas.','Regularize ou atualize a vigência e execute uma nova verificação.'],'actions'=>[['label'=>'Abrir assinaturas','url'=>'/billing'],['label'=>'Ver empresas','url'=>'/companies']]],
            ...
            'after_hours_recovery' => ['title'=>'Recuperação pós-horário requer revisão','cause'=>'Uma conversa preservada fora do horário não conseguiu concluir a retomada automática.','impact'=>'A mensagem continua armazenada, mas pode aguardar uma nova tentativa até a causa ser corrigida.','steps'=>['Abra a Fila da IA e confira as pendências pós-horário.','Valide se a empresa já está em horário comercial e se a conversa continua em modo IA.','Confira franquia de IA, credencial e conexão WhatsApp.','Após corrigir a causa, execute nova verificação do sistema; a recuperação reutiliza a demanda original e evita duplicidade.'],'actions'=>[['label'=>'Abrir fila da IA','url'=>'/central-operacao?tab=ai_reprocess'],['label'=>'Ver conversas','url'=>'/conversations'],['label'=>'Ver assinatura','url'=>'/subscription']]],
