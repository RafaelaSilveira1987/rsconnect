...
$checks = [
    ...
    'cadastro possui fallback seguro' => str_contains($signup, "Env::get('ASAAS_USER_AGENT', 'RS-Connect/36.24.5')")
    ...
    'gateway financeiro injeta User-Agent' => str_contains($payment, '$headers = $this->withAsaasUserAgent($headers);')
        && str_contains($payment, "Env::get('ASAAS_USER_AGENT', 'RS-Connect/36.24.5')"),
    'versão do pacote atualizada' => str_contains($version, 'RS Connect 36.24.5')
        && str_contains($signup, "public const VERSION = '36.24.5';"),
    'variável opcional documentada' => str_contains($env, 'ASAAS_USER_AGENT=RS-Connect/36.24.5'),
];
