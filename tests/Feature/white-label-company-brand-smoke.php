...
$check(str_contains($view, 'name="brand_logo_background"'), 'tela permite escolher a cor de fundo da logo');
...
$check(str_contains($controller, 'white_label_enabled = :enabled') && str_contains($controller, 'brand_logo_url = :brand_logo_url') && str_contains($controller, 'brand_logo_background = :brand_logo_background'), 'persistência limita a personalização à logo e ao fundo do quadro');
$check(str_contains($controller, '$pdo->beginTransaction()'), 'gravação da logo usa transação');
$check(str_contains($controller, 'SELECT white_label_enabled, brand_logo_url, brand_logo_background'), 'controller relê a configuração depois de salvar');
...
$check(str_contains($branding, "'logo_background' => \$logoBackground"), 'serviço aplica a cor de fundo da logo');
...
$check(($sample['logo_background'] ?? '') === '#f4f7fb', 'execução real devolve a cor de fundo persistida');
