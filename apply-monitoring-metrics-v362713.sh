...
BACKUP_DIR="$TARGET_ROOT/storage/backups/monitoring-commercial-n8n-v362713-$STAMP"
...
FILES=(
  ...
  "tests/Feature/operations-monitor-workflow-identity-v362713-smoke.php"
...
Exemplo:
  mkdir -p /tmp/rsconnect-v362713
  unzip rs-connect-v36.27.13-monitor-id-unico.zip -d /tmp/rsconnect-v362713
  cd /tmp/rsconnect-v362713/rs-connect-v36.27.13-monitor-id-unico
  bash apply-monitoring-metrics-v362713.sh /var/www/html
EOF
  exit 2
fi
...
echo "[1/6] Validando sintaxe PHP..."
for relative in "${FILES[@]}"; do
...
echo "[2/6] Validando regras comerciais e métricas n8n..."
php "$TARGET_ROOT/tests/Feature/operational-commercial-n8n-v362711-smoke.php"

echo "[3/6] Validando proteção do Monitor operacional..."
php "$TARGET_ROOT/tests/Feature/operations-monitor-workflow-autoreactivate-v362712-smoke.php"

echo "[4/6] Validando identidade única do Monitor operacional..."
php "$TARGET_ROOT/tests/Feature/operations-monitor-workflow-identity-v362713-smoke.php"

echo "[5/6] Reativando/publicando o workflow crítico no n8n, se necessário..."
php "$TARGET_ROOT/bin/ensure-operations-monitor-workflow.php" --activate

echo "[6/6] Auditando fontes reais e exigindo Monitor operacional único e identificado..."
php "$TARGET_ROOT/bin/monitoring-source-audit.php" --require-n8n-live --require-monitor-active --require-monitor-id

trap - ERR

echo
printf '%s\n' "[APROVADO] RS Connect 36.27.13 aplicado; Monitor operacional confirmado como único, identificado, publicado e agendado."
