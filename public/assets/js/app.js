...
(function () {
  const liveUrl = document.body?.dataset?.globalLiveUrl || '';
  if (!liveUrl) return;

  let timer = null;
  let previousConversationUnread = null;

  function setBadge(type, count) {
    document.querySelectorAll(`[data-global-badge="${type}"]`).forEach((badge) => {
  ...
  function updateDashboard(payload) {
    ...
    Object.keys(metrics).forEach((key) => {
      document.querySelectorAll(`[data-dashboard-live="${key}"]`).forEach((node) => {
        node.textContent = Number(metrics[key] || 0);
        node.closest('.metric-card')?.classList.add('metric-pulse');
        window.setTimeout(() => node.closest('.metric-card')?.classList.remove('metric-pulse'), 500);
      });
    });

    const list = document.querySelector('[data-dashboard-notifications-list]');
    if (!list || !Array.isArray(payload.notifications)) return;
    if (payload.notifications.length < 1) return;

    list.innerHTML = payload.notifications.slice(0, 3).map((item) => {
      const href = item.action_url || '/notifications';
      const unread = item.status === 'unread' ? ' is-unread' : '';
      return `<a class="compact-notification${unread}" href="${escapeHtml(href)}">
        <span class="notification-dot"></span>
        <span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.message).slice(0, 120)}</small></span>
      </a>`;
    }).join('');
  }
  ...
  async function pollGlobal() {
    try {
      ...
      const conversationUnread = Number(payload.conversation_unread || 0);
      const notificationUnread = Number(payload.notification_unread || 0);

      setBadge('conversation', conversationUnread);
      setBadge('notification', notificationUnread);
      updateDashboard(payload);

      if (previousConversationUnread !== null && conversationUnread > previousConversationUnread && !location.pathname.includes('/conversations')) {
        document.dispatchEvent(new CustomEvent('rsconnect:new-message', { detail: { count: conversationUnread } }));
      }
      previousConversationUnread = conversationUnread;
    } catch (error) {
      // Silencioso: a tela principal continua funcionando mesmo se a atualização global falhar.
    } finally {
      window.clearTimeout(timer);
      timer = window.setTimeout(pollGlobal, document.hidden ? 18000 : 7000);
    }
  }

  document.addEventListener('visibilitychange', () => {
    if (!document.hidden) pollGlobal();
  });
